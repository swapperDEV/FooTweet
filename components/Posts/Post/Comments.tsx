  ...
  const pushComment = () => {
    if (commentRef.current!.value.length > 3) {
      ...
      sendNotify(
        ...
        `${userCtx.data.username} comment your post`,
        "comment"
      );
        ...
        <>
          {commentCreateView && (
            <div className={postStyles.commentCreate}>
              <div>
                <input placeholder="comment post" ref={commentRef} />
                <button onClick={() => pushComment()}>Push</button>
              </div>
            </div>
          )}
          <div className={postStyles.comments}>
            {post.data.interaction.comments.map((comment) => {
              return (
                <Comment
