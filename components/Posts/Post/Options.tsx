...
export const Options = ({
  ...
  const [isUserRetweet, updateIsUserRetweet] = useState<undefined | boolean>(
    post.data.retweets.includes(fbCtx.currentUser.uid)
  );
  const UserCtx = useContext(UserDataContext);
  let likesNumber: Array<string> = post.data.interaction.likes;
  const updateDB = (table: Array<string>) => {
  ...
  const likePost = () => {
    ...
    updateDB(table);
    sendNotify(
      post.data.creator.uId,
      UserCtx.data.username,
      `${UserCtx.data.username} like your post.`,
      "like"
    );
  };
  const dislikePost = () => {
  ...
  const sharePost = () => {
    navigator.clipboard.writeText(
      window.location.origin + `/post/${post.data.metaData.postId}`
    );
  ...
  return (
    <>
      <div className={postStyles.options}>
        ...
        <div>
          <FaRetweet
            onClick={() => postRetweet(post.data.metaData.postId)}
            className={
              isUserRetweet ? postStyles.retweetActive : postStyles.none
            }
          />
        </div>
        <div>
