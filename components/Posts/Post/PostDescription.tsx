    ...
    <>
      <div className={postStyles.info}>
        <div className={postStyles.user}>
          ...
          <p
            className={postStyles.username}
          >{`@${post.data.creator.username}`}</p>
        </div>
        <p className={postStyles.infoR}>
          <FaClock />
          {time}
        </p>
        {fbCtx.currentUser.uid === post.data.creator.uId && (
          <p onClick={() => openSettings()} className={postStyles.settings}>
            <FaEllipsisH />
          </p>
        )}
