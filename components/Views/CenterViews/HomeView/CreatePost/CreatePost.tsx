...
export const CreatePost = () => {
  ...
  const [hashtagStyles, setHashtagStyles] = useState(
    createPostStyles.hashtagsList
  );
    ...
    userCtx.data.followers.forEach((follower: Array<string>) => {
      getUserData(follower).then((data) => {
        ...
        sendNotify(
          ...
          `${userCtx.data.username} push a new post!`,
          "newpost"
        );
      });
    });
  ...
  return (
    <>
      ...
      <div className={createPostStyles.createPost}>
        ...
        <div
          className={createPostStyles.createPostRight}
          onMouseEnter={handleOpenPostTools}
          onMouseLeave={handleClosePostTools}
        >
          <textarea
            ref={postContentRef}
            onChange={handleWrite}
            placeholder="What is happening?"
            className={createPostStyles.inputText}
          />
          {postTools && (
            <div className={createPostStyles.animate}>
              ...
              {photoPreview && (
                <FaWindowClose
                  onClick={() => delImage()}
                  className={createPostStyles.close}
                />
              )}
              <div className={createPostStyles.postSettings}>
                  ...
                  <div>
                    <label className={createPostStyles.filebutton}>
                      ...
                      <span>
                        <input
                          onClick={handleWrite}
                          className={createPostStyles.file}
                          accept="image/*"
                          type="file"
                          value={imgRef}
                ...
                <div className={createPostStyles.hashtags}>
                  <div className={hashtagStyles}>
                    Hasztag list <br />
                    <ul>
                      {hashtag.map((h: String, index: number) => (
                        <li key={index}>{`#${h}`}</li>
                      ))}
                    </ul>
                    <button onClick={() => closeHashtagList()}>close</button>{" "}
                    <button onClick={() => resetHashtagList()}>reset</button>
                  </div>
                  <input
