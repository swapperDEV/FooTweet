...
export const Followed = ({
  ...
  const unFollowUser = async (username: string, uid: string) => {
    unfollowUser(username, uid, followedUsersTable, id, yourUsername).then(
      (value) => {
      ...
      <div className={followedStyles.wrapper}>
        <div className={followedStyles.following}>
          {
            <>
              ...
              {followedUsersTable.length !== 0 ? (
                <>
                  <p className={followedStyles.info}>
                    {userCtx.data.username === yourUsername
                      ? ` The users you follow`
                      : `The users followed by ${yourUsername}`}
                  </p>
                  {followedUsersTable.map((user) => {
                    return (
                      <FollowedUser
                        check1={userCtx.data.username}
                        check2={yourUsername}
                        user={user}
                        key={user}
        ...
        {userCtx.data.username === yourUsername && (
          <div className={followedStyles.wrapperRight}>
            <Suggestions
              followedUsers={followedUsersTable}
              id={id}
