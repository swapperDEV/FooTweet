...
export const MobileMenu = () => {
    ...
    return (
        ...
        <div className={navDisplay ? `${mobileStyles.navbar} ${mobileStyles.navbarUnHide}` : mobileStyles.navbar}>
            <div className={mobileStyles.userBaner} onClick={() => redirectToProfile()}>
            ...
            <div>
                <ul>
                    <li onClick={() => Navigate('home')} className={path.pathname === '/home' ? mobileStyles.active : ''}><FaHome/><p>Home</p></li>
                    <li onClick={() => Navigate('hashtag')} className={hashtag ? mobileStyles.active : ''}><FaHashtag/><p>Hashtags</p></li>
                    <li onClick={() => Navigate('search')} className={search ? mobileStyles.active : ''}><FaSearch/><p>Search</p></li>
                    <li onClick={() => Navigate('notifications')} className={path.pathname === '/notifications' ? mobileStyles.active : ''}><FaBell/><p>Notifications ({notifyList !== undefined ? notifyList.length : 0})</p></li>
                    <li onClick={() => Navigate('profile')} className={profile ? mobileStyles.active : ''}><FaUser/><p>Profile</p></li>
                    <li onClick={() => Navigate('messages')} className={messages ? mobileStyles.actView : ''}><FaFacebookMessenger/><p>Messages</p></li>
                    <li><FaAlignLeft/><p onClick={() => logoutAccount()}>Logout</p></li>
                </ul>
            ...
            <div className={mobileStyles.center}>
                <div><FaSearch/><input ref={searchRef} onKeyDown={(event:any) => searchByKeyWords(event)} placeholder="Search on FooTweet" maxLength={20}/></div>
            </div>
        </div>
        </>
    )
